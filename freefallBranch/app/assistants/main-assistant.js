...
var freefallCookie = ({
	initialize: function() {
		...
		if (storedData && storedData.version == "1.0.1") {
			global.reviewed = storedData.reviewed;
		}
		this.storeCookie();
	},
...
MainAssistant.prototype.initialPrompt = function () {
	this.controller.showAlertDialog({
		...
		title: $L("Freefall!"),
		message: $L("I've been getting a lot of negative reviews, so if you're one of the few who actually enjoy the game please help me out & let people know by giving it a good review!  Thanks in advance!"),
		choices: [{
			label: $L('Ok (Opens a new card)'),
			value: 'yes',
			type: 'affirmative'
		}, {
			label: $L('No thanks'),
	...
	if (choice == 'yes') {
		this.controller.serviceRequest("palm://com.palm.applicationManager", {
			...
			parameters: {
				...
				params: {
					target: "http://developer.palm.com/appredirect/?packageid=net.bradleygraber.freefallfree"
				}
			}
		});
	}
...
MainAssistant.prototype.setup = function(){
	...
	this.controller.setupWidget('hideButton', {
	...
	this.controller.setupWidget('difficultyButton', {
		type: Mojo.Widget.defaultButton
	}, global.difficultyButtonModel);
	...
	this.controller.setupWidget('highScoresButton', {
		type: Mojo.Widget.defaultButton
	}, local.highScoreButtonModel);
	...
	this.controller.setupWidget('supportButton', {
		type: Mojo.Widget.defaultButton
	}, local.supportButtonModel);
	...
	this.controller.setupWidget('goButton', {
		type: Mojo.Widget.defaultButton
	}, local.goButtonModel);
	...
	this.tapBuyButtonListener = this.tapBuyButton.bindAsEventListener(this)
	this.navListener = this.nav.bindAsEventListener(this)
	this.checkAccelListener = this.checkAccel.bindAsEventListener(this)
	this.tapGoButtonListener = this.tapGoButton.bindAsEventListener(this)
	this.tapHighScoresButtonListener = this.tapHighScoresButton.bindAsEventListener(this)
	this.tapHideButtonListener = this.tapHideButton.bindAsEventListener(this)
	this.tapDifficultyButtonListener = this.tapDifficultyButton.bindAsEventListener(this)
	this.tapSupportButtonListener = this.tapSupportButton.bindAsEventListener(this)
	this.pauseListener = this.pause.bindAsEventListener(this)
	
	this.controller.listen(elements.buyButton, Mojo.Event.tap, this.tapBuyButtonListener)
	this.controller.listen(elements.appHeader,Mojo.Event.tap, this.navListener);
	this.controller.listen(document, 'acceleration', this.checkAccelListener);
	this.controller.listen(elements.goButton, Mojo.Event.tap, this.tapGoButtonListener);
	this.controller.listen(elements.highScoresButton, Mojo.Event.tap, this.tapHighScoresButtonListener);
	this.controller.listen(elements.hideButton, Mojo.Event.tap, this.tapHideButtonListener);
	this.controller.listen(elements.difficultyButton, Mojo.Event.tap, this.tapDifficultyButtonListener);
	this.controller.listen(elements.supportButton, Mojo.Event.tap, this.tapSupportButtonListener);
	this.controller.listen(elements.pause, Mojo.Event.tap, this.pauseListener);
...
MainAssistant.prototype.cleanup = function(event) {
	...
	this.controller.stopListening(elements.buyButton, Mojo.Event.tap, this.tapBuyButtonListener)
	this.controller.stopListening(elements.appHeader,Mojo.Event.tap, this.navListener);
	this.controller.stopListening(document, 'acceleration', this.checkAccelListener);
	this.controller.stopListening(elements.goButton, Mojo.Event.tap, this.tapGoButtonListener);
	this.controller.stopListening(elements.highScoresButton, Mojo.Event.tap, this.tapHighScoresButtonListener);
	this.controller.stopListening(elements.hideButton, Mojo.Event.tap, this.tapHideButtonListener);
	this.controller.stopListening(elements.difficultyButton, Mojo.Event.tap, this.tapDifficultyButtonListener);
	this.controller.stopListening(elements.supportButton, Mojo.Event.tap, this.tapSupportButtonListener);
	this.controller.stopListening(elements.pause, Mojo.Event.tap, this.pauseListener);
