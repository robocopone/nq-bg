...
long	survivingCols( expvec * M, long * surviving )
{
   long nrSurv = 0, h = 1, i;
...
void	outputMatrix( expvec * M, char * suffix )
{
...
expvec*	MatrixToExpVecs()
{
   ...
   if ( NrRows == 0 ) {
      ...
      TimeOutOff();
      if ( Gap ) 
         printGapMatrix( (expvec*)0 );
      if ( AbelianInv ) 
         outputMatrix( (expvec*)0, "abinv" );
      if ( RawMatOutput && RawMatFile != NULL ) 
         fclose( RawMatFile );
      TimeOutOn();
      return (expvec*)0;
   }
...
void	vSubOnce( lvec v, lvec w, long a )
{
   while ( a <= NrCols ) {
      mpz_sub( v[a], w[a], v[a] ); 
      a++;
...
void	vSub( lvec v, lvec w, long a )
{
   mpz_t q, t;
...
int	addRow( expvec ev )
{
   ...
   */
   ...
   if ( RawMatOutput ) {
      for ( i = 1; i <= NrCols; i++ )
         fprintf( RawMatFile, " %d", (int) ev[NrPcGens+i] );
      fprintf( RawMatFile, "\n" );
      fflush( RawMatFile );
   }
   ...
   if ( NrRows == NrCenGens ) {
      /* Check to see if each leading entry is 1 */
      for ( i = 0; i < NrRows; i++ ) 
         if ( mpz_sizeinbase( Matrix[i][Heads[i]], 2 ) != 1 ) 
            break;
      if ( i == NrRows ) 
         EarlyStop = 1;
   }
...
/************************************************************************
** $Log: glimt.c,v $
** Revision 1.8  2008-11-12 19:25:02  bg41
** Style changes -bkg
**
** Revision 1.7  2008-09-30 17:53:50  bg41
** changed some %d to %ld and cast some exp types to ints -bkg
**
** Revision 1.6  2008-09-30 16:52:06  bg41
** removed a couple unused variables -bkg
**
** Revision 1.5  2008-09-26 15:55:27  bg41
** added prototypes for TimeOutOn() & TimeOutOff() -bkg
**
** Revision 1.4  2008-09-25 19:21:03  bg41
** modified header -bkg
**
** Revision 1.3  2008-09-25 19:03:40  bg41
** #include <stdlib> added to fix incompatible implicit 
** declaration warning -bkg
**
** Revision 1.2  2008-09-25 19:00:40  bg41
** Added header & footer -bkg
*/
