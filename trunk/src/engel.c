...
void	buildPairs( word u, long i, gen g, word v, long wt, long which )
{
   ...
   */
   ...
   u[i].g = EOW;
   u[i].e = (exp)0;
   if( EarlyStop || SemigroupOnly || !SemigroupFirst )
      return;
...
void	evalLREngel()
{
   ...
   for( n = 1; !EarlyStop && n <= NrEngelGens; n++ ) {
      ...
      if( RevEngel ) 
         A[0].g = NumberOfAbstractGens() - n + 1;
      else
         A[0].g = n;
      A[0].e = (exp)1;
      A[1].g = EOW; 
      ...
      for( cl = 2; !EarlyStop && Needed && cl <= Class+1; cl++ ) {
         ...
         NrWords = 0;
         if(Verbose) 
            printf("#    Checking words of weight %ld\n",cl-1);
...
/************************************************************************
** $Log: engel.c,v $
** Revision 1.5  2008-11-12 17:18:53  bg41
** Style changes -bkg
**
** Revision 1.4  2008-11-06 15:11:37  bg41
** Style changes -bkg
**
** Revision 1.3  2008-09-30 17:59:36  bg41
** removed an unused variable
** changed some %d to %ld -bkg
**
** Revision 1.2  2008-09-26 17:01:50  bg41
** updated header, added footer
** added prototype for addRow()
** added prototype for NumberOfAbstractGens(); -bkg
*/
